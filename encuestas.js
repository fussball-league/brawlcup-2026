                            ...
                            <div class="grupo-encuesta ${estaAbierta ? '' : 'encuesta-bloqueada'}">
                                ...
                                <section class="contenedor-votos">
                                    ${partida.opciones.map(opc => `
                                        <div class="opcion-voto ${estaAbierta ? '' : 'deshabilitada'}" 
                                             ${estaAbierta ? `onclick="votar('${opc.id}')"` : ''}>
                                            ${opc.imagen ? `<img src="${opc.imagen}" class="opcion-logo" style="${estaAbierta ? '' : 'filter: grayscale(1); opacity: 0.5;'}">` : ''}
                                            <div class="opcion-info">
                                                <h3>${opc.nombre}</h3>
                                                <p>${estaAbierta ? 'Haz clic para votar' : '<i class="fa-solid fa-lock"></i> Próximamente'}</p>
                                            </div>
                                            ${estaAbierta ? '<i class="fa-solid fa-chevron-right icon-flecha"></i>' : ''}
                                        </div>
                                    `).join('')}
                                </section>
                            </div>
